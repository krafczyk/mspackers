...
int main(int argc, char** argv) {
    ...
    ArgParse::ArgParser Parser("kwaj_unpack");
    ...
    struct mskwaj_decompressor* decompressor = mspack_create_kwaj_decompressor(NULL);
    if(decompressor == NULL) {
        std::cerr << "There was a problem creating the decompressor" << std::endl;
        return 1;
    }

    // Decompress
    int result = decompressor->decompress(decompressor, input_filepath.c_str(), output_filepath.c_str());
    if(result != MSPACK_ERR_OK) {
        std::cerr << "There was an error decompressing!" << std::endl;
